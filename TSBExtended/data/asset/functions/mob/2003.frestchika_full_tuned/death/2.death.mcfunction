...
# アマスタに位置をあわせる
    execute at @e[type=armor_stand,tag=1JN.ArmorStand,distance=..3,sort=nearest,limit=1] run tp @e[type=armor_stand,tag=1JN.ArmorStandThis,distance=..3] @s
# タグを消す
    tag @e[type=armor_stand,tag=1JN.ArmorStandThis] remove 1JN.ArmorStandThis
...
# スケジュールループを開始する
    schedule function asset:mob/2003.frestchika_full_tuned/death/schedule 1t