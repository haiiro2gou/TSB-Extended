...
# CompareResult = Array.map(v => v.id === CompareTarget)
    ...
    data modify storage lib: CompareTarget set from storage api: Argument.ReduceEnchantment
...
# エンチャント係数を取得する
    ...
    execute unless score $TypeModifier Temporary matches -2147483648.. if data storage api: Argument{ReduceEnchantment:      "fire_protection"} run scoreboard players set $TypeModifier Temporary 2
    execute unless score $TypeModifier Temporary matches -2147483648.. if data storage api: Argument{ReduceEnchantment:     "blast_protection"} run scoreboard players set $TypeModifier Temporary 2
    execute unless score $TypeModifier Temporary matches -2147483648.. if data storage api: Argument{ReduceEnchantment:"projectile_protection"} run scoreboard players set $TypeModifier Temporary 2
    execute unless score $TypeModifier Temporary matches -2147483648.. if data storage api: Argument{ReduceEnchantment:      "feather_falling"} run scoreboard players set $TypeModifier Temporary 3
