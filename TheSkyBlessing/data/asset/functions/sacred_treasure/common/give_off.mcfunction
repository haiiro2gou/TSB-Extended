#> asset:sacred_treasure/common/give_off
#
# 神器を入手します
#
...
# 神器排出
    ...
    execute unless data storage asset:context {Type:"drop"} if entity @s[nbt={Inventory:[{Slot:-106b}]}] run loot spawn ~ ~ ~ mine 10000 0 10000 debug_stick
    execute unless data storage asset:context {Type:"drop"} unless entity @s[nbt={Inventory:[{Slot:-106b}]}] run loot replace entity @s weapon.offhand mine 10000 0 10000 debug_stick
    execute unless data storage asset:context {Type:"drop"} if entity @s[nbt={Inventory:[{Slot:-106b}]}] as @e[type=item,nbt={Item:{tag:{TSB:{}}}},distance=..0.3] run function asset:sacred_treasure/common/protect
# リセット
