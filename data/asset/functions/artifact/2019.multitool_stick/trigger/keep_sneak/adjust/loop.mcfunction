#> asset:artifact/2019.multitool_stick/trigger/keep_sneak/adjust/loop
#
# ループしてブロックサーチ
#
# @within function
#   asset:artifact/2019.multitool_stick/trigger/keep_sneak/adjust/
#   asset:artifact/2019.multitool_stick/trigger/keep_sneak/adjust/loop
...
# 割当て
    ...
    execute unless score $1K3.Tool Temporary matches 1.. if block ^ ^ ^ #asset:artifact/2019.multitool_stick/shears run scoreboard players set $1K3.Tool Temporary 5
    execute unless score $1K3.Tool Temporary matches 1.. unless block ^ ^ ^ air run scoreboard players set $1K3.Tool Temporary 0

# ループ
    execute unless score $1K3.Tool Temporary matches 1.. positioned ^ ^ ^0.1 if entity @a[tag=1K3.Marker,distance=..5] run function asset:artifact/2019.multitool_stick/trigger/keep_sneak/adjust/loop