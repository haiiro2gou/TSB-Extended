#> asset:artifact/2009.desert_eagle_empty/trigger/1.trigger
#
...
# storage asset:idのmainhand/offhandに装備している神器のIDが入っているので比較し、~/2.check_condition.mcfunctionを実行する
    ...
    execute if entity @s[tag=1JT.Main] run function asset:artifact/2009.desert_eagle_empty/trigger/2.check_condition
    tag @s remove 1JT.Main
    execute if data storage asset:context id{offhand:2009} run tag @s add 1JT.Off
    execute if entity @s[tag=1JT.Off] run function asset:artifact/2009.desert_eagle_empty/trigger/2.check_condition
