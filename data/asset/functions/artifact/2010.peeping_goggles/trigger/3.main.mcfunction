...
# 神器のメイン処理部
#
# @within function asset:artifact/2010.peeping_goggles/trigger/2.check_condition
...
# 初期設定
    execute if score $1JU.Rarity Temporary matches 1 run data modify storage lib: Array set from storage asset:artifact RarityRegistry[1]
    execute if score $1JU.Rarity Temporary matches 2 run data modify storage lib: Array set from storage asset:artifact RarityRegistry[2]
    execute if score $1JU.Rarity Temporary matches 3 run data modify storage lib: Array set from storage asset:artifact RarityRegistry[3]
    execute if score $1JU.Rarity Temporary matches 4 run data modify storage lib: Array set from storage asset:artifact RarityRegistry[4]
...
# 神器の名前を取得
    execute store result score $Argument.Index Lib run function asset:artifact/2010.peeping_goggles/trigger/random
...
# 代償
    ...
    execute if score @s 1JU.CoolTime matches 1.. run function asset:artifact/2010.peeping_goggles/trigger/damage
...
# ループ
    ...
    schedule function asset:artifact/2010.peeping_goggles/trigger/schedule_loop 1t replace
