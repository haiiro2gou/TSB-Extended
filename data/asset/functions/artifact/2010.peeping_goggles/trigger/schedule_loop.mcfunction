...
#   asset:artifact/2010.peeping_goggles/trigger/schedule_loop
#   asset:artifact/2010.peeping_goggles/trigger/rejoin_process

# CoolTime
    execute as @a[scores={1JU.CoolTime=1..}] run scoreboard players remove @s 1JU.CoolTime 1
    execute as @a[scores={1JU.CoolTime=70}] at @s run function asset:artifact/2010.peeping_goggles/trigger/damage
...
# ループ
    execute if entity @a[scores={1JU.CoolTime=1..},limit=1] run schedule function asset:artifact/2010.peeping_goggles/trigger/schedule_loop 1t replace