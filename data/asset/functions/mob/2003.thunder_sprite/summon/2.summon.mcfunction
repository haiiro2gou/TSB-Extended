#> asset:mob/2003.thunder_sprite/summon/2.summon
#
# Mobの召喚時の処理
#
# @within function asset:mob/2003.thunder_sprite/summon/1.trigger

# 元となるMobを召喚する
    summon vex ~ ~ ~ {Tags:["MobInit","1JN.Sprite","AlwaysInvisible"],DeathLootTable:"asset:mob/death/2003.thunder_sprite"}
# ID (int)
...
# 名前 (TextComponentString) (オプション)
    data modify storage asset:mob Name set value '{"text":"サンダースプライト","color":"aqua"}'
# 武器
...
# ノックバック耐性 (double) (オプション)
    data modify storage asset:mob KnockBackResist set value 10
# 属性倍率 // 1.0fで100% 最低でも25%は軽減されずに入る
    # 物理倍率 (float) (オプション)
        data modify storage asset:mob Resist.Physical set value 0.8
    # 魔法倍率 (float) (オプション)
        data modify storage asset:mob Resist.Magic set value 0.2
    # 火倍率 (float) (オプション)
...
# スコア設定
    execute as @e[type=vex,tag=MobInit,distance=..0.01] run scoreboard players set @s 1JN.Tick 300
