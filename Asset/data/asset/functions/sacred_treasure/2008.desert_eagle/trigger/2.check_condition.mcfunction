...
# 神器の基本的な条件の確認を行うfunction、成功している場合CanUsedタグが付く
    execute as @s[tag=!1JS.Off] run function asset:sacred_treasure/common/check_condition/mainhand
    execute as @s[tag=1JS.Off] run function asset:sacred_treasure/common/check_condition/offhand
# 他にアイテム等確認する場合はここに書く
    # 火薬の所持チェック
        ...
        # execute if score $1JS.GunpowderCount Temporary matches 0 run function lib:message/sacred_treasure/dont_have_require_items
        # scoreboard players reset $1JS.GunpowderCount Temporary

# CanUsedタグをチェックして3.main.mcfunctionを実行する
    execute if entity @s[tag=CanUsed] run function asset:sacred_treasure/2008.desert_eagle/trigger/3.main